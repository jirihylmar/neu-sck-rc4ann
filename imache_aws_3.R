...
#system ('aws s3 cp s3://neuroappinputbucket-dev/staana.alfa1@gmail.com ~/datastore/AlfaDWbh6/ext-imareg/rc2_20190828 --recursive') # !!! set corresponding images and outfolder ----

# transfer from s3 - data ----
#system ('aws s3 cp s3://neu-rc2-dyn-alf/lim-csv-2 ~/datastore/AlfaDWbh6/ext-imareg/rc2_csv_2 --recursive')


###_______________________====
# DATA ----
df.path = "~/datastore/AlfaDWbh6/ext-imareg/rc2_csv_2"
...
# IMAGES CHECKING ----
# #checking ----
# df.path = "~/datastore/AlfaDWbh6/ext-imareg/rc2_20190828/staana.alfa1@gmail.com"
...
# df.files.che = df.files.che %>% filter(mtimeday != "2019-08-28")
# 
# ## remove files ----
...
# 
# df.files.aws = gsub('/home/hylmarj/datastore/AlfaDWbh6/ext-imareg/rc2_20190828', #~ REPLACE WITH FULL PATH
...
df.files.id = 
c(
# df.files.id [grepl("masB01",df.files.id)][1:49],
# df.files.id [grepl("masC02",df.files.id)],
# df.files.id [grepl("masC03",df.files.id)],
df.files.id [grepl("masF04",df.files.id)],
df.files.id [grepl("masP05",df.files.id)]
# 
# df.files.id [grepl("masB01",df.files.id)][50:301],
# df.files.id [grepl("masF04",df.files.id)][16:126],
# df.files.id [grepl("masP05",df.files.id)][6:42]
)
...
# 
   #e= grep ("staana.alfa1@gmail.com/masB01/20190702095433/norori/bdecb4ca-9c9e-11e9-a885-005056840793.png",df.files.id) #debug Type1----
#   
#   e= grep ("staana.alfa1@gmail.com/masB01/20190804171150/norori/accf461c-b6ca-11e9-ad58-005056840793.png",df.files.id) #debugType2----
#   
  # 
  #select images ----
  ...
  tit.mai =gsub ('staana.alfa1@gmail.com',"",ima.uid)
  tit.mai =gsub ('norori',"norsta",tit.mai)
  image.norsta = implot(image.norsta,text(250,50,tit.mai,cex=1))
  
  tit.mai =gsub ('staana.alfa1@gmail.com',"",ima.uid)
  tit.mai =gsub ('norori',"invcro",tit.mai)
  image.invcro = implot(image.invcro,text(250,30,tit.mai,cex=1,col='white'))
  
  tit.mai =gsub ('staana.alfa1@gmail.com',"",ima.uid)
  tit.mai =gsub ('norori',"invsta",tit.mai)
  image.invsta = implot(image.invsta,text(250,30,tit.mai,cex=1,col='white'))
  ...
  path.outDir.ful = paste0( "~/datastore/AlfaDWbh6/ext-imareg/rc2_20190901_draw",
                            "/",
                            str.nam.cre[1],
                            "/",
                            str.nam.cre[2],
  ...
  if (TES == 0) { 
    ...
    path.outDir = "~/datastore/AlfaDWbh6/ext-imareg/rc2_20190901_draw"
  }
  ...
  png(path.outDir.ful,width = 1920, height = 1080) # !!! comment not to save image ----
  ...
  pri.rat.x = c(.01,.27,.01,.29,.01,.29,.01,.1,.01)
  pri.rat.y = c(.01,.49,.49,.01)
  
  sum(pri.rat.x)
  sum(pri.rat.y)
  
  layout(layout.matrix,
         heights =pri.rat.y, # Heights  rows
         widths = pri.rat.x
  ) # Widths columns
...
###||||||||||||||||||||||||||||||||||||||||||||||====
  ...
  # invsta ----
  plot(0,type='n',axes=FALSE,ann=FALSE)
  plot(1-image.invsta,axes = FALSE)
  n.y = height(image.invsta) / (height(image.invsta)/160*10)
  n.x = width(image.invsta)/ (height(image.invsta)/160*10)
  vec.x = seq(0, width(image.invsta), length.out = n.x)
  vec.y <- seq(0, height(image.invsta), length.out = n.y)
  ...
  text(
    10,
    vec.y+height(image.invsta) /(n.y-1)/2,
    seq( from = 1, to = length(vec.y)-1 ),
    col = "grey",
    cex = cex.tex
  ...
  text(
    vec.x+width(image.invsta) /(n.x-1)/2,
    10,
    LETTERS[seq( from = 1, to = length(vec.x)-1 )],
    col = "grey",
    cex = cex.tex
  ...
  n.y = height(image.invcro) / (height(image.invcro)/160*5)/df.pl.tra.imareg$imareg_norcro_trasca
  n.x = height(image.invcro) / (height(image.invcro)/160*5)/df.pl.tra.imareg$imareg_norcro_trasca
  len.x = height(image.invcro)/n.x
  len.y = len.x
  vec.x = seq(0, width(image.invcro), length.out = n.x)
  vec.y <- seq(0, height(image.invcro), length.out = n.y)
  abline(
    h=vec.y,
    col = "lightgray", 
    lty = "dotted",
  ...
  text(
    10,
    vec.y+height(image.invcro) /(n.y-1)/2,
    seq( from = 1, to = length(vec.y)-1 ),
    col = "grey",
    cex = cex.tex
  ...
  text(
    vec.x+width(image.invcro) /(n.x-1)/2,
    10,
    LETTERS[seq( from = 1, to = length(vec.x)-2 )],
    col = "grey",
    cex = cex.tex
  ...
  # plot.new()
  plot(0,type='n',axes=FALSE,ann=FALSE)
  plot.window(xlim=c(0,190), ylim=c(height(image.invsta),0),ann=FALSE)
  
  n.y = (height(image.invsta)/160*5)
  vec.y <- seq(0, height(image.invsta), length.out = n.y)
